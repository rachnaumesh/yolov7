...
FILE = Path(__file__).resolve()  # noqa: E402
ROOT = FILE.parents[1]  # YOLOv5 root directory # noqa: E402
if str(ROOT) not in sys.path:  # noqa: E402
    sys.path.append(str(ROOT))  # add ROOT to PATH # noqa: E402
ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # relative # noqa: E402
...
def run_segment(
        ...
        img_array=None,  # numpy array for image
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det[:, :6]):
                    ...
                    seg_boxes.append(np.array(line_tensor))
                    conf_scores.append(conf)
                
                # print(f'number of boxes {len(det[:, :6])} and masks {len(masks)}')
#                 print(f'masks are : {masks.detach().cpu().numpy()}')
#                 print(f'boxes are : {seg_boxes}')
    ...
    return masks, seg_boxes, conf_scores
